...
# set outfile, errfile
# query these from ${outfiles}, ${errfiles}
outfile=$(sed -e "${line_number}q;d" ${outfiles})
errfile=$(sed -e "${line_number}q;d" ${errfiles})
...
command $(sed -e "${line_number}q;d" ${command_file}) > ${outfile} 2> ${errfile}
